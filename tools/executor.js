...
// Management cycle sets this to true so duplicate close/deploy/swap
// notifications are suppressed — the management report already includes them.
let _suppressNotifications = false;
export function setSuppressNotifications(v) { _suppressNotifications = !!v; }
    ...
    if (success) {
      if (!_suppressNotifications) {
        if (name === "swap_token" && result.tx) {
          try { notifySwap({ inputSymbol: args.input_mint?.slice(0, 8), outputSymbol: args.output_mint === "So11111111111111111111111111111111111111112" || args.output_mint === "SOL" ? "SOL" : args.output_mint?.slice(0, 8), amountIn: result.amount_in, amountOut: result.amount_out, tx: result.tx }); } catch {}
        } else if (name === "deploy_position") {
          try { notifyDeploy({ pair: result.pool_name || args.pool_name || args.pool_address?.slice(0, 8), amountSol: args.amount_y ?? args.amount_sol ?? 0, position: result.position, tx: result.txs?.[0] ?? result.tx, priceRange: result.price_range, rangeCoverage: result.range_coverage, binStep: result.bin_step, baseFee: result.base_fee }); } catch {}
        } else if (name === "close_position") {
          try { notifyClose({ pair: result.pool_name || args.position_address?.slice(0, 8), pnlUsd: result.pnl_usd ?? 0, pnlPct: result.pnl_pct ?? 0, reason: args.reason }); } catch {}
        }
      }
